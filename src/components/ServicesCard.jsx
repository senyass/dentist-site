...
function ServicesCard({image, title, description, servicesIds, flip=false, setCurrentPage, setSelectedServiceId}) {
    ...
    return (
        <div className="bg-alice-blue grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-30 ">
    ...
    />
                    ...
                    <div className={`flex flex-col h-full justify-center px-10 ${flip ? "lg:order-1" : ""}`}>
                        ...
                        <div className="grid grid-cols-2 gap-x-10 gap-y-10 mt-12 text-left pl-5">
                            {servicesIds.map((id) => {
                                const service = servicesData.find((service) => service.id === id);
                                if (!service) return null;

                                return (
                                    <div key={service.id} className="flex items-center gap-4 hover:text-pacific-blue">
                                        <img src={tooth} alt="Tooth" className="w-8 h-8"/> 
                                        <button className="!text-[23px]" onClick={() => { setSelectedServiceId(service.id); setCurrentPage("service-page")}}>
                                            {service.title}
                                        </button>
                                    </div>
                                ) ;
